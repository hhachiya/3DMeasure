...
def get_seg(val, color_array):
    ...
    id = np.argmin(dif)
    return id, color_array[id]/255.0 # RGB値の取得
...
def get_segment_image(img, mask, id_color):
    ...
    segRGB = np.zeros([img.shape[0],img.shape[1],3], dtype=np.float)
    
    for j in range(h):
        for i in range(w):
            if mask[j,i] > 0:
                seg[j,i], segRGB[j,i] = get_seg(img[j,i], id_color) # +1はH4がゼロ無いから
                seg[j,i] += 1
...
def decode_x(cap_v, mask, color_array, id_color):
    seg_img, seg_img_RGB = get_segment_image(cap_v, mask, id_color)
    h, w = mask.shape[:2]
    ...
    return decoded_img, seg_img_RGB

def decode_y(cap_h, mask, color_array, id_color):
    transposed_h = np.transpose(cap_h, (1,0,2))
    transposed_m = np.transpose(mask,(1,0))
    decoded_v,seg_img_RGB = decode_x(transposed_h, transposed_m, color_array, id_color)
    decoded_h = np.transpose(decoded_v, (1,0))
    seg_img_RGB = np.transpose(seg_img_RGB, (1,0,2))
    
    return decoded_h,seg_img_RGB
...
if __name__ == '__main__':
    ...
    parser.add_argument('--rgb_thr', '-rgb_thr', type=int, default='150')
    ...
    for w_file, x_file, y_file in zip(white_files, x_files, y_files):
        ...
        decoded_x,seg_img_RGB_x = decode_x(cap_x, mask, color_array, id_color)
        decoded_y,seg_img_RGB_y = decode_y(cap_y, mask, color_array, id_color)
        ...
        #-----------------
        # カメラ画像、選択した色、選択したindexおよびマスク画像の比較表示
        output_8bit_x_file = os.path.join(output_8bit_dir, os.path.splitext(w_file)[0] + '_x_RGB.png')
        output_8bit_y_file = os.path.join(output_8bit_dir, os.path.splitext(w_file)[0] + '_y_RGB.png')
        
        fig = plt.figure()
        ax = fig.add_subplot(2,2,1)        
        ax.imshow(cap_x[:,:,[2,1,0]]/255.0)
        ax.set_title("original")
        
        ax = fig.add_subplot(2,2,2)
        ax.imshow(decoded_x,cmap="gray")
        ax.set_title("selected code index")
        
        ax = fig.add_subplot(2,2,3)
        ax.imshow(seg_img_RGB_x[:,:,[2,1,0]])
        ax.set_title("selected color")
        
        ax = fig.add_subplot(2,2,4)
        ax.imshow(mask,cmap="gray")
        ax.set_title("mask image")

        plt.savefig(output_8bit_x_file)
        
        fig = plt.figure()
        ax = fig.add_subplot(2,2,1)        
        ax.imshow(cap_y[:,:,[2,1,0]]/255.0)
        ax.set_title("original")
        
        ax = fig.add_subplot(2,2,2)
        ax.imshow(decoded_y,cmap="gray")        
        ax.set_title("selected code index")

        ax = fig.add_subplot(2,2,3)
        plt.imshow(seg_img_RGB_y[:,:,[2,1,0]])
        ax.set_title("selected color")
        ...
        plt.savefig(output_8bit_y_file)
        #-----------------
